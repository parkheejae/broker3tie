...
/*
 * 15초마다 db의 stock 테입르의 정보를 실시간으로 가져와서 broker의 stockList에 뿌려주는 기능을 추가
 * 쓰레드로 작성 Broker
 * 
 */
	...
	*/
	...
	//==================================================================
		...
		*argument로 받은 CustomerRec[]을 stockList에 하나씩 뿌려준다.<BR>
		...
		public void showCustomer()throws Exception{
			...
			StringTokenizer st = new StringTokenizer(str);//공백을 기준으로 자른다.
		...
		public void addCustomer(){
			...
			try {
				db.addCustomer(cr);
				showCustList(db.getAllCustomers(),custList);//이부분 반드시
			}catch(Exception e) {
				System.out.println(e.getMessage());
			}
		...
		public void updateCustomer(){
			...
			try{
				...
				showCustList(db.getAllCustomers(),custList);//이부분 반드시 추가함..
			}catch(Exception e) {
				System.out.println(e.getMessage());
			}			
	    } 
	...
	//=============================================================
		...
		public void itemStateChanged(ItemEvent ie) {
			...
			if(list.equals(custList)) { //고객의 정보를 출력하는 리스트
				try {
					showCustomer();
				} catch (Exception e) {
			...
			}else if(list.equals(stockList)) {//주식의 정보를 나타내는 리스트..
				showStock();
			}else {
				showPortfolio();
			}			
	...
	public void run() {
		while(true) {
			try {
			showStockList(db.getAllStocks(),stockList);
			System.out.println("실시간 주식정보 공유");
			Thread.sleep(15000);
			}catch(Exception e) {
